...
try {
    ...
    if ($null -eq $process) {
        #run the upgrade
        sysget upgrade -e --id $ID --silent --accept-package-agreements --accept-source-agreements --uninstall-previous # https://github.com/microsoft/winget-pkgs/issues/232739
        #rechecks the version if it installed and creates values for final output.
        $lines = sysget list --accept-source-agreements --Id $ID
    }
    else {
